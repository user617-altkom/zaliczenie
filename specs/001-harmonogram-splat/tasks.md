...
- [ ] T001 Zdefiniować typy publiczne domeny (`TypRat`, `KodWskaznika`, `WpisWskaznika`, `SeriaWskaznika`, `TrybNadplaty`, `Nadplata`, `ParametryKredytu`, `PozycjaHarmonogramu`, `Harmonogram`) w `src/domena/harmonogram.ts` zgodnie z [data-model.md](./data-model.md); wszystkie pola kwotowe w groszach jako `number` (int), stopa jako `number` (ułamek roczny > 0 i < 1); brak importów Reacta i I/O.
- [ ] T002 [P] Napisać test `tests/dane/wskazniki.test.ts` sprawdzający: (a) `pobierzSerie('polstr-1m')` zwraca `SeriaWskaznika` z niepustymi `wpisy` posortowanymi rosnąco po `data`; (b) `pobierzSerie('wibor-3m')` – jak wyżej; (c) niespójna seria (duplikat daty) rzuca `Error`. Test najpierw czerwony.
- [ ] T003 Zaimplementować `src/dane/wskazniki.ts`: statyczny import `dane/polstr-1m.json` i `dane/wibor-3m.json`, funkcja `pobierzSerie(kod: KodWskaznika): SeriaWskaznika`, walidacja kształtu (kolejność, brak duplikatów, wartości > 0 i < 1). Nie edytujemy plików w `dane/` (Konstytucja § „Ograniczenia techniczne i stos”). T002 musi być zielony po T003.
- [ ] T004 [P] Napisać test `tests/domena/stopaNaOkres.test.ts` sprawdzający `stopaNaOkres`: (a) dla daty raty przed pierwszym wpisem zwraca wartość pierwszego wpisu; (b) dla daty raty po ostatnim wpisie zwraca wartość ostatniego wpisu (FR-005); (c) dla daty raty między wpisami zwraca wartość najpóźniejszego wpisu ≤ data raty (FR-004); (d) do zwróconej stopy dodawana jest marża (`marzaPp / 100`).
- [ ] T005 Zaimplementować pomocnicze funkcje domeny w `src/domena/harmonogram.ts`: `stopaNaOkres(seria, dataRaty, marzaPp)` (zgodna z D2/D3 z [research.md](./research.md)) oraz `nastepnaDataRaty(dataPierwszejRaty, numer)` dodająca `numer` miesięcy kalendarzowo bez `Date.now()`. T004 zielony po T005.
